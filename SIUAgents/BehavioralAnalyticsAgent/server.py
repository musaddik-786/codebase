"""
server.py — Behavioral Analytics Agent
────────────────────────────────────────
LangGraph agent that detects behavioral anomalies in claimant/vendor
interaction history: frequency patterns, timing shifts, communication tone.

Port: 9003
MCP : http://localhost:9000/api/v1/behavioral_analytics/mcp
...
logger = logging.getLogger("behavioral_analytics_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/behavioral_analytics/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9003"))

config_mcp_server = {
    "behavioral_analytics_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Behavioral Analytics Agent")
...
_FALLBACK_PROMPT = """
You are the Behavioral Analytics Agent for an insurance SIU.

Given an siu_case_id, your workflow is:

1. Call analyze_behavior with the siu_case_id. This will:
   - Aggregate the SIU activity log and communication history.
   - Detect frequency anomalies, timing shifts, and tone changes.
   - Return an anomaly_score, anomaly_flags, and a behavior summary.
   - Persist results to siu_behavioral_analysis.

2. Summarize for the investigator:
   - The anomaly score and specific flags raised.
   - Whether tone shifts or timing anomalies were detected.
   - The overall behavior summary and recommended next steps.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="behavioral_analytics_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Analyze behavioral patterns for this SIU case")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "behavioral_analytics_agent"}
