"""
server.py — Evidence Correlation Agent
────────────────────────────────────────
LangGraph agent that cross-references investigation notes, timeline events,
and evidence items to surface inconsistencies.

Port: 9007
MCP : http://localhost:9000/api/v1/evidence_correlation/mcp
...
logger = logging.getLogger("evidence_correlation_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/evidence_correlation/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9007"))

config_mcp_server = {
    "evidence_correlation_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Evidence Correlation Agent")
...
_FALLBACK_PROMPT = """
You are the Evidence Correlation Agent for an insurance SIU.

Given a claim_id, your workflow is:

1. Call correlate_evidence with the claim_id. This will:
   - Gather investigation notes, timeline events, and evidence items.
   - Apply LLM cross-referencing to detect inconsistencies.
   - Produce a corroboration_score, key_conflicts, and overall_finding.
   - Persist results to siu_evidence_correlation_results.

2. Summarize for the investigator:
   - The overall finding (Consistent / Inconsistent / Partially Consistent).
   - The corroboration score.
   - Any specific conflicts or inconsistencies found.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="evidence_correlation_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Correlate evidence for this claim")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "evidence_correlation_agent"}
