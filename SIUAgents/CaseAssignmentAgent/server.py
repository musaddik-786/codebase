"""
server.py — Case Assignment Agent
───────────────────────────────────
LangGraph agent that assigns an SIU investigator to a claim using
skill-based and workload-balanced matching.

Port: 9002
MCP : http://localhost:9000/api/v1/case_assignment/mcp
...
logger = logging.getLogger("case_assignment_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/case_assignment/mcp")
...
app = FastAPI(title="Case Assignment Agent")
...
_FALLBACK_PROMPT = """
You are the Case Assignment Agent for an insurance SIU (Special Investigations Unit).

Given a claim_id, your workflow is:

1. Call assign_investigator with the claim_id. This will:
   - Evaluate the claim's loss type and fraud score.
   - Match the claim to the best available investigator by specialty and workload.
   - Write the assignment to siu_case_master.

2. Summarize for the SIU supervisor:
   - Which investigator was assigned and their specialty.
   - The assignment type (Standard / Specialist / Senior).
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="case_assignment_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Assign an investigator to this SIU case")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "case_assignment_agent"}
