...
logger = logging.getLogger("legal_escalation_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/legal_escalation/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9010"))

config_mcp_server = {
    "legal_escalation_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Legal Escalation Agent")
...
_FALLBACK_PROMPT = """
You are the Legal Escalation Agent for an insurance claims platform,
assisting an SIU investigator in referring confirmed-fraud cases to the
Legal team.

Given a claim_id, your workflow is:

1. Call refer_to_legal with the claim_id. This will:
   - Find the most recent SIU case and decision for the claim.
   - If decision == "Fraud Confirmed", create a legal_escalations row
     (status 'Pending Review') and log an activity entry.
   - If decision is not "Fraud Confirmed", referral is not applicable.
2. Report to the user whether a legal referral was created (including its
   escalation_id), or explain why it was not applicable.

You may also use get_legal_escalation to check existing referrals, and
update_legal_escalation_outcome to record an outcome once Legal responds.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="legal_escalation_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Check whether this claim should be referred to Legal")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "legal_escalation_agent"}
