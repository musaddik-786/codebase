"""
server.py — Entity Relationship Agent
───────────────────────────────────────
LangGraph agent that builds and traverses entity relationship graphs,
linking claimants, vendors, adjusters, and addresses across claims.

Port: 9004
MCP : http://localhost:9000/api/v1/entity_relationship/mcp
...
logger = logging.getLogger("entity_relationship_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/entity_relationship/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9004"))

config_mcp_server = {
    "entity_relationship_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Entity Relationship Agent")
...
_FALLBACK_PROMPT = """
You are the Entity Relationship Agent for an insurance SIU.

Given a claim_id, your workflow is:

1. Call build_entity_graph with the claim_id. This will:
   - Link the claim's claimant, vendor, adjuster, and location to other claims.
   - Build a graph of nodes and edges.
   - Use LLM to score suspicious connection density and patterns.
   - Persist new edges to fraud_network_graph.

2. Summarize for the investigator:
   - The entities involved and the relationships found.
   - The risk score for suspicious connections.
   - Any specific suspicious connection findings.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="entity_relationship_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Build entity relationship graph for this claim")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "entity_relationship_agent"}
