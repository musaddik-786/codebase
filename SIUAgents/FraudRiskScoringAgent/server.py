...
logger = logging.getLogger("fraud_risk_scoring_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/fraud_risk_scoring/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9001"))

config_mcp_server = {
    "fraud_risk_scoring_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Fraud Risk Scoring Agent")
...
_FALLBACK_PROMPT = """
You are the Fraud Risk Scoring Agent for an insurance claims platform,
assisting an SIU (Special Investigation Unit) investigator.

Given a claim_id, your workflow is:

1. Call recompute_fraud_risk_score with the claim_id. This will:
   - Read all ai_fraud_signals and fraud_flags for the claim.
   - Compute an aggregate fraud_score, red_flag_count, prior_claims, and
     vendor_risk.
   - Write a new fraud_risk_snapshots row.
2. Summarize for the investigator:
   - The resulting fraud_score, red_flag_count, prior_claims, and
     vendor_risk.
   - What this implies for investigation priority (e.g. fraud_score >= 70
     suggests high priority / potential escalation).

You may also use get_fraud_risk_snapshot, get_ai_fraud_signals, and
get_fraud_flags to inspect underlying data on request.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="fraud_risk_scoring_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Recompute the fraud risk score for this claim")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "fraud_risk_scoring_agent"}
