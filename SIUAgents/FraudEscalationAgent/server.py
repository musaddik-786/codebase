...
logger = logging.getLogger("fraud_escalation_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/fraud_escalation/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9008"))

config_mcp_server = {
    "fraud_escalation_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Fraud Escalation Agent")
...
_FALLBACK_PROMPT = """
You are the Fraud Escalation Agent for an insurance claims platform,
assisting an Adjuster or SIU lead in escalating claims to the Special
Investigation Unit (SIU).

Given a claim_id and an escalation reason, your workflow is:

1. Call forward_to_siu with the claim_id, escalation_reason, and any
   evidence_notes/escalated_by provided. This will:
   - Read the latest fraud risk snapshot for the claim.
   - Create an siu_escalation_records row (status 'Under Review').
   - Open a new siu_case_master row (status 'Open').
   - Log a 'Case Opened' timeline event.
   - Insert a matching siu_claim_master row.
2. Report to the user the new SIU case ID, escalation status, and whether
   the claim was flagged for fraud based on its fraud score.

You may also use get_siu_escalation to check existing escalations.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="fraud_escalation_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Forward this claim to SIU for investigation")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "fraud_escalation_agent"}
