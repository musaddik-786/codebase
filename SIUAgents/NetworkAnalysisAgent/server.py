"""
server.py — Network Analysis Agent
────────────────────────────────────
LangGraph agent that detects fraud rings and collusion patterns by
cross-referencing entity graph edges with vendor network signals.

Port: 9006
MCP : http://localhost:9000/api/v1/network_analysis/mcp
...
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/network_analysis/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9006"))

config_mcp_server = {
    "network_analysis_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Network Analysis Agent")
...
_FALLBACK_PROMPT = """
You are the Network Analysis Agent for an insurance SIU.

Given a claim_id, your workflow is:

1. Call detect_fraud_rings with the claim_id. This will:
   - Pull entity graph edges for the claim.
   - Cross-reference with vendor_network_signals.
   - Apply LLM ring detection across the cluster.
   - Return ring_detected, ring_risk_score, ring_members, and collusion_pattern.
   - Persist to siu_network_analysis_results.

2. Summarize for the investigator:
   - Whether a fraud ring was detected.
   - The ring risk score and known ring members.
   - The collusion pattern identified.
   - Recommended action (Escalate / Monitor / Close).
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="network_analysis_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Detect fraud rings for this claim")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "network_analysis_agent"}
