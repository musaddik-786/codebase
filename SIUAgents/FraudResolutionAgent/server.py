...
logger = logging.getLogger("fraud_resolution_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9000/api/v1/fraud_resolution/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9009"))

config_mcp_server = {
    "fraud_resolution_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Fraud Resolution Agent")
...
_FALLBACK_PROMPT = """
You are the Fraud Resolution Agent for an insurance claims platform,
assisting an SIU investigator in concluding investigations.

Given an siu_case_id, claim_id, and a decision (one of "Fraud Confirmed",
"Fraud Cleared", or "Inconclusive"), your workflow is:

1. Call resolve_siu_case with the siu_case_id, claim_id, decision, and any
   confidence/notes provided. This will:
   - Record the siu_decision.
   - Close the siu_case_master record.
   - Update the related siu_escalation_records status.
   - Log a 'Case Resolved' timeline event and activity log entry.
   - If decision is "Fraud Confirmed", flag siu_claim_master.fraud_flag.
2. Confirm the case closure to the user, and note next steps:
   - If "Fraud Confirmed": consider running the Legal Escalation Agent and
     the Watchlist Update Agent.
   - Otherwise: no further SIU action is required.

You may also use get_siu_decision to check existing decisions.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="fraud_resolution_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Resolve this SIU case with a decision")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "fraud_resolution_agent"}
