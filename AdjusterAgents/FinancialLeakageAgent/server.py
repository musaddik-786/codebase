"""
server.py — Financial Leakage Agent
─────────────────────────────────────
LangGraph agent that detects overpayments and billing anomalies by scoring
financial leakage risk across vendor line items on a claim.

Port: 8911
MCP : http://localhost:8900/api/v1/financial_leakage/mcp
...
logger = logging.getLogger("financial_leakage_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:8900/api/v1/financial_leakage/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "8911"))

config_mcp_server = {
    "financial_leakage_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Financial Leakage Agent")
...
def router(state: State):
    ...
    # Only route to tools when there are actual tool_calls — string matching on
    # "Continue"/"End" is unreliable if the LLM uses those words in normal text.
...
_FALLBACK_PROMPT = """
You are the Financial Leakage Agent for an insurance claims platform,
assisting a Claims Adjuster.

Given a claim_id, your workflow is:

1. Call the tool named "score_leakage" exactly (use that exact name) with the claim_id.
   This will:
   - Aggregate estimated vs. actual costs across all vendors on the claim.
   - Compute a leakage score (0-100) and an overall variance percentage.
   - Use an LLM to identify specific overpayment risk flags per vendor.

2. Summarize for the adjuster:
   - Total estimated vs. actual costs and the variance.
   - The leakage score and risk level (Low/Medium/High/Critical).
   - Specific vendor risk flags and the recommended next steps.
   - If no vendor cost data is found, report that clearly instead of guessing.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="financial_leakage_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Score financial leakage for this claim")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "financial_leakage_agent"}
