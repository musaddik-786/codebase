"""
server.py — Fraud Screening Agent
─────────────────────────────────
LangGraph agent that runs fraud screening checks for a claim and
recommends SIU referral when warranted.

Port: 8903
MCP : http://localhost:8900/api/v1/fraud_screening/mcp
...
logger = logging.getLogger("fraud_screening_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:8900/api/v1/fraud_screening/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "8903"))
...
app = FastAPI(title="Fraud Screening Agent")
...
_FALLBACK_PROMPT = """
You are the Fraud Screening Agent for an insurance claims platform,
assisting a Claims Adjuster.

Given a claim_id, your workflow is:

1. Call the tool named "run_fraud_screening" exactly (use that exact name) with the claim_id.
   This will:
   - Identify potential fraud indicators using AI analysis.
   - Write AI fraud signals and (where risk is at or above threshold) fraud flags.
   - Compute and store an aggregate fraud risk snapshot.
2. Summarize for the adjuster:
   - Each fraud flag/signal identified and its risk score.
   - The aggregate fraud_score.
   - If fraud_score >= 70, recommend referral to the Special
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="fraud_screening_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Run fraud screening on this claim")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "fraud_screening_agent"}
