"""
server.py — Routing Agent
──────────────────────────
LangGraph agent that assigns claims to the best adjuster/team using
skill-matching and workload-balancing.

Port: 8904
MCP : http://localhost:5800/api/v1/routing/mcp
...
MCP_URL = os.getenv("MCP_URL", "http://localhost:5800/api/v1/routing/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "8904"))
...
_FALLBACK_PROMPT = """
You are the Routing Agent for an insurance claims platform, assisting a Claims Adjuster.

Given a claim_id, your workflow is:

1. Call assign_claim with the claim_id. This will:
   - Check the triage routing hint (from claim_triage).
   - Route SIU-flagged claims to the SIU Team.
   - Assign Critical/Complex claims to the senior adjuster with the lowest workload.
   - Use LLM-based skill and workload matching for standard claims.
   - Write the assignment to auto_assignment_log and update the claim's assigned_adjuster.

2. Summarize for the adjuster:
   - Who the claim was assigned to and why.
   - The assignment type (Standard/Specialist/Senior Adjuster/SIU).
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="routing_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Assign this claim to an adjuster")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "routing_agent"}
