"""
server.py — Vendor Qualification Agent
────────────────────────────────────────
LangGraph agent that scores vendors against compliance criteria (license,
insurance, certifications, background check) and updates assignment_eligible.

Port: 9103
MCP : http://localhost:9100/api/v1/vendor_qualification/mcp
...
logger = logging.getLogger("vendor_qualification_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9100/api/v1/vendor_qualification/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9103"))

config_mcp_server = {
    "vendor_qualification_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Vendor Qualification Agent")
...
_FALLBACK_PROMPT = """
You are the Vendor Qualification Agent for an insurance claims platform,
assisting a Vendor Manager.

Given a vendor_id, your workflow is:

1. Call score_vendor_qualification with the vendor_id. This will:
   - Check license validity and expiry date.
   - Verify insurance documentation, certifications, and background check status.
   - Compute a qualification score (0-100%).
   - Use an LLM to determine assignment_eligible (Yes/No/Conditional).
   - Update assignment_eligible in vendor_master_input.

2. Summarize for the Vendor Manager:
   - The qualification score and compliance status for each check.
   - The assignment eligibility verdict and risk level.
   - Any failed compliance checks and recommended remediation steps.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="vendor_qualification_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Score qualification for this vendor")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "vendor_qualification_agent"}
