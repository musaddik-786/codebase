"""
server.py — Vendor Matching Agent
─────────────────────────────────
LangGraph agent.

Port: 9102
MCP : http://localhost:9100/api/v1/vendor_matching/mcp
...
logger = logging.getLogger("vendor_matching_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9100/api/v1/vendor_matching/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9102"))
...
app = FastAPI(title="Vendor Matching Agent")
...
_FALLBACK_PROMPT = """
You are the Vendor Matching Agent for an insurance claims platform,
assisting an Adjuster or Vendor Manager in finding and assigning vendors to
claims.

Your workflow:

1. Given a claim_id, call match_vendor_for_claim to get the top 3 candidate
   vendors (ranked by rating and turnaround time) for the claim's loss type
   and location.
2. Present the candidates to the user, including their vendor_id, name,
   specialty, rating, and avg_turnaround_days.
3. If the user confirms a vendor, call assign_vendor_to_claim with the
   claim_id, vendor_id, and vendor_type (specialty) to record the
   assignment (assignment_status 'Assigned', sla_status 'On Track').

You may also use get_vendors and get_vendor_master to look up vendor details
directly.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="vendor_matching_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Find and assign the best vendor for claim CLM-2026-1001")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "vendor_matching_agent"}
