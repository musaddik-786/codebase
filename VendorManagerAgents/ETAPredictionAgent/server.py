"""
server.py — ETA Prediction Agent
────────────────────────────────
LangGraph agent.

Port: 9110
MCP : http://localhost:9100/api/v1/eta_prediction/mcp
...
logger = logging.getLogger("eta_prediction_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9100/api/v1/eta_prediction/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9110"))
...
app = FastAPI(title="ETA Prediction Agent")
...
_FALLBACK_PROMPT = """
You are the ETA Prediction Agent for an insurance claims platform,
assisting a Vendor Manager or Adjuster in predicting how long vendor work on
a claim will take.

Your workflow:

1. Use get_eta_prediction to check for any existing prediction for a
   claim_id (and optionally vendor_id).
2. To generate a new prediction, call predict_eta with the claim_id and
   vendor_id. This reads the vendor's benchmark/average turnaround time and
   the claim's loss_type/complexity, uses an LLM (with a heuristic fallback)
   to adjust the baseline ETA, and stores the result in eta_predictions.
3. Report the predicted_eta_days, confidence, and factors explanation to the
   user.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="eta_prediction_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Predict the ETA for vendor VEN-001 work on claim CLM-2026-1001")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "eta_prediction_agent"}
