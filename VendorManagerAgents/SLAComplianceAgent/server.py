"""
server.py — SLA Compliance Agent
────────────────────────────────
LangGraph agent.

Port: 9108
MCP : http://localhost:9100/api/v1/sla_compliance/mcp
...
logger = logging.getLogger("sla_compliance_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9100/api/v1/sla_compliance/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9108"))
...
app = FastAPI(title="SLA Compliance Agent")
...
_FALLBACK_PROMPT = """
You are the SLA Compliance Agent for an insurance claims platform,
assisting a Vendor Manager in checking a vendor's SLA compliance.

Your workflow:

1. Use get_vendor_jobs_sla to review a vendor's jobs (vendor_jobs_input) and
   their sla_status values.
2. Call compute_sla_compliance with the vendor_id. This computes the
   percentage of jobs that are not 'Overdue' (sla_compliance), derives
   placeholder avg_response_time / avg_completion_time from the vendor's
   avg_turnaround_days, and writes/updates an sla_tracker_output row.
3. Use get_sla_tracker to retrieve the latest stored SLA tracker record for
   a vendor.
4. Report the SLA compliance percentage and average response/completion
   times to the user.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="sla_compliance_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Check SLA compliance for vendor VEN-001")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "sla_compliance_agent"}
