"""
server.py — Vendor Escalation Agent
───────────────────────────────────
LangGraph agent.

Port: 9109
MCP : http://localhost:9100/api/v1/vendor_escalation/mcp
...
logger = logging.getLogger("vendor_escalation_agent")

PHOENIX_API_KEY = os.getenv("PHOENIX_API_KEY", "")
PHOENIX_ENDPOINT = os.getenv("PHOENIX_ENDPOINT", "")
MCP_URL = os.getenv("MCP_URL", "http://localhost:9100/api/v1/vendor_escalation/mcp")
AGENT_PORT = int(os.getenv("AGENT_PORT", "9109"))

config_mcp_server = {
    "vendor_escalation_mcp": {
        "url": MCP_URL,
...
app = FastAPI(title="Vendor Escalation Agent")
...
_FALLBACK_PROMPT = """
You are the Vendor Escalation Agent for an insurance claims platform,
assisting a Vendor Manager in identifying and escalating overdue vendor
jobs.

Your workflow:

1. To check for overdue jobs (optionally for a specific vendor_id), call
   escalate_overdue_jobs. This scans vendor_jobs_input for jobs with
   sla_status 'Overdue' and active 'Yes', and for each one:
   - Creates an escalation_log_output row (severity 'High').
   - Inserts/updates a job_status_update_output row (escalation_flag 'Yes',
     priority 'High') for the related claim.
   Report the list of escalations created.
2. Use get_vendor_escalations to look up existing escalation records by
   vendor_id and/or claim_id.
3. To manually log an escalation (not derived from an overdue job), call
   create_vendor_escalation with claim_id, vendor_id, severity, message, and
   optional created_by.
...
def load_prompt() -> str:
    ...
    prompt = client.prompts.get(name="vendor_escalation_agent", label="production")
...
async def chat_stream(request: Request):
    ...
    user_message = body.get("message", "Check for overdue vendor jobs and escalate them")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "vendor_escalation_agent"}
